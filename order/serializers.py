...
class SalesOfAnAgentSerializer(serializers.ModelSerializer):
    ...
    def get_total_sale(self, obj):
        ...
        if this_week:
            ...
            total_amount = OrderItem.objects.filter(product__user__agent_user_id=obj.id, suborder__payment_status='PAID', date__range=(week_start,week_end)).aggregate(total=Sum('total_price'))['total'] or 0
        elif this_month:
            ...
            total_amount = OrderItem.objects.filter(product__user__agent_user_id=obj.id, suborder__payment_status='PAID', date__range=(first,last)).aggregate(total=Sum('total_price'))['total'] or 0
        else:
            # total_amount = SubOrder.objects.filter(farmer__agent_user_id=obj.id, status='PAID').aggregate(total=Sum('amount'))['total'] or 0
