...
def unique_order_id_generator_for_order(instance, new_order_id=None):
    if new_order_id is not None:
        ...
        order_id_str = instance.delivery_address.district.english_name[:3] + "-" + str(random_id_generator(size=6))
...
def unique_order_id_generator_for_suborder(instance, new_order_id=None):
    if new_order_id is not None:
        ...
        order_id_str = instance.delivery_address.district.english_name[:3] + "-" + str(random_id_generator(size=8))
        order_id = slugify(str(order_id_str))
    Klass = instance.__class__
